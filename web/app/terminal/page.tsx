...
function BookRows({ side, rows }: { side: "ask" | "bid"; rows: Level[] }) {
  ...
  return <div className={`book-rows ${side}`}>{levels.map(([price, size]) => {
    const style = { "--book-depth": `${Math.max(8, (size / maxSize) * 100)}%` } as CSSProperties;
    return <div className="book-row" key={price} style={style}><span>{number.format(price)}</span><span>{number.format(size)}</span></div>;
  })}</div>;
}
...
export default function TerminalPage() {
  ...
  const loadMarket = useCallback(async () => {
    ...
    try {
      ...
      if (requestController.current === controller) setMarket(data);
    } catch (requestError) {
  ...
  const status = loading ? "LOADING" : error ? "RETRYING" : "LIVE DATA";
  const statusTone = error ? "error" : "live";
