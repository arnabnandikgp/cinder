...
|---|---|---|---|
| **G-PNL** | **Trading PnL on reducing ack (incl. liq ack).** `ack_phoenix_fill` realizes `-(entry_closed + vwap)` for `OID_PENDING` and `OID_LIQUIDATING`. `fill_quote == 0` does not invent PnL. Fail-ack does not realize. G-DEBT leftover / venue-liq socialize (G-SOC) still out. | Demo walkthrough needs a true close. Full insurance/socialize is not this slice. | Reducing-ack + liq-ack realize implemented. G-SOC/G-DEBT remain. |
