...
|---|---|---|---|
| **G-PNL** | **Trading PnL on reducing ack.** `ack_phoenix_fill` realizes `-(entry_closed + vwap)` into free then reserved when the fill reduces the position. `fill_quote == 0` does not invent PnL. Fail-ack does not realize. Liq-path realize and G-DEBT leftover are still out. | Demo walkthrough needs a true close. Full insurance/socialize is not this slice. | Reducing-ack implemented (`realize_on_fill`). Liquidate still does not realize. |
